...
export const textSchema = z.object({
    prompt: z.string().min(0, {message: "Minimum prompt limit is atleast 2"}).max(1000 , "Maximum prompt words limit is 1000"),
    model: z.enum(models),
    type: z.number().int().min(0 , {message: "Invalid type"}).max((modelPrompts.length)-1  , {message: "Invalid type"})
})